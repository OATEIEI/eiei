...
if(isset($_POST['submit'])){
    ...
    $num = mysqli_fetch_array($con->query("SELECT pro_name  FROM product WHERE pro_name='$proname'"));
    if($num == 1){
        echo "<script>alert('สินค้านี้มีอยู่เเล้วมีอยู่เเล้วเปลี่ยนซะ')</script>";
    }else{
    $sql = "INSERT INTO product (pro_name,pro_price,pro_amount,pro_status) VALUES('$proname','$proprice','$proamount','$prostatus')";
